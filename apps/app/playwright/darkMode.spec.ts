...
test("Dark Mode Tests", () => {
  ...
  test("should toggle to dark mode when clicking the dark mode button", async ({
    page,
  }) => {
  ...
  test("should toggle back to light mode when clicking the dark mode button twice", async ({
    page,
  }) => {
    // Initially should be in light mode
    await expect(page.locator("html")).not.toHaveClass(/dark/);
  ...
  test("should show the correct icon based on the current theme", async ({
    page,
  }) => {
  ...
  test("should persist dark mode preference after page reload", async ({
    page,
  }) => {
    // Switch to dark mode
    await page
